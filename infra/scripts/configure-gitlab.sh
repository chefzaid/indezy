...
if ! { printf '%s\n' "$vault_token"; } | \
  kubectl exec -i -n "$INFRA_NAMESPACE" "$VAULT_POD" -- sh -ceu '
    IFS= read -r VAULT_TOKEN
    export VAULT_TOKEN VAULT_ADDR=http://127.0.0.1:8200
    vault kv get secret/apps/indezy/runtime >/dev/null 2>&1
  '; then
  info "Creating Indezy database and JWT secrets in Vault"
  database_password="$(openssl rand -hex 24)"
  jwt_secret="$(openssl rand -hex 48)"
  { printf '%s\n%s\n%s\n' "$vault_token" "$database_password" "$jwt_secret"; } | \
    kubectl exec -i -n "$INFRA_NAMESPACE" "$VAULT_POD" -- sh -ceu '
      IFS= read -r VAULT_TOKEN
      IFS= read -r database_password
      IFS= read -r jwt_secret
      export VAULT_TOKEN VAULT_ADDR=http://127.0.0.1:8200
      vault kv put secret/apps/indezy/runtime \
        database_password="$database_password" jwt_secret="$jwt_secret" >/dev/null
    '
  unset database_password jwt_secret
fi

git -C "$REPOSITORY_ROOT" show HEAD:.gitlab-ci.yml >/dev/null
kubectl apply -f "$REPOSITORY_ROOT/infra/argocd/indezy-app.yaml"
if kubectl get externalsecret indezy-registry-auth -n "$APP_NAMESPACE" >/dev/null 2>&1; then
  kubectl annotate externalsecret indezy-registry-auth -n "$APP_NAMESPACE" \
