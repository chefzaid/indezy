...
# Check indezy-server dependencies
check_backend() {
    if [ -f "/workspace/indezy-server/pom.xml" ]; then
...
# Check indezy-web dependencies
check_frontend() {
    if [ -f "/workspace/indezy-web/package.json" ]; then
        print_status "Frontend project found"
        if [ -d "/workspace/indezy-web/node_modules" ]; then
...
main() {
    ...
    if [ $failed_checks -eq 0 ]; then
        ...
        echo "  • indezy-server  - Start indezy-server server"
        echo "  • indezy-web - Start indezy-web server"
